...
def ISLAM_find_next_one(my_list, index):
    DATE_REGEX = r'\d{1,2}/\d{2}/\d{2}'
    AMOUNT_REGEX = r'\.\d{2}'
    BAL_REGEX = r'\.\d{2}'
    for i in range(index + 1, len(my_list)):
        elements = my_list[i].split()
        if my_list[i] == '(RM) (RM) (RM)':
            if 'BALB/F' not in my_list[i+1]:
                return i + 1
            else:
                return i + 2
...
def ISLAM_process_rows(rows, bal, sort):
    DATE_REGEX = r'\d{1,2}/\d{2}/\d{2}'
    AMOUNT_REGEX = r'\.\d{2}'
    BAL_REGEX = r'\.\d{2}'
...
def ISLAM_main(rows, bal, sort):
    ...
    for index in indices_containing:
        if 0 <= index < len(rows):
            result_index = ISLAM_find_next_one(rows, index)
            if result_index != -1:
    ...
    if sort == '-1':
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                diff = row['Balance'] - round(float(previous_balance), 2)
                A = str(row['Amount'])
                if diff < 0:
